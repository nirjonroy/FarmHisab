...
<div class="row g-3">
        ...
        <select id="farm_id" name="farm_id" class="form-select @error('farm_id') is-invalid @enderror" required>
            ...
            @foreach ($farms as $farm)
                <option value="{{ $farm->id }}" @selected((int) old('farm_id', $shed->farm_id ?? '') === $farm->id)>
                    {{ $farm->name }} @if (! $farm->is_active) (Inactive) @endif
                </option>
            @endforeach
        </select>
    ...
    <div class="col-md-6">
        <label for="code" class="form-label">Shed code</label>
...
<div class="mt-4 d-flex gap-2">
    ...
    <a href="{{ route('sheds.index') }}" class="btn btn-outline-secondary">Cancel</a>
</div>
