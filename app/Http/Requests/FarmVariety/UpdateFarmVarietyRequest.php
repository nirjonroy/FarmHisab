...
class UpdateFarmVarietyRequest extends FormRequest
{
    ...
    {
        $variety = $this->route('farmVariety');
        $name = $this->input('name_en') ?: $this->input('name_bn') ?: '';
        $slug = $this->filled('slug')
            ? Str::slug($this->input('slug'))
            : $this->uniqueSlug(Str::slug($name) ?: 'variety', $variety?->id);
    ...
    public function rules(): array
    {
        ...
        return [
            ...
            'slug' => ['nullable', 'string', 'max:150', Rule::unique('farm_varieties', 'slug')->ignore($variety)],
            'code' => [
                'nullable',
                'string',
                'max:50',
                Rule::unique('farm_varieties', 'code')
                    ->where('farm_category_id', $this->input('farm_category_id'))
                    ->ignore($variety),
            ],
    ...
    private function uniqueSlug(string $slug, ?int $ignoreId): string
    {
        ...
        while (FarmVariety::where('slug', $candidate)->when($ignoreId, fn ($query) => $query->whereKeyNot($ignoreId))->exists()) {
